...
5. Выберите в списке, **что переносить**, и нажмите **«Перенести из Pixso»**.
...
**Что переносить — три варианта:**

| выбор | что берётся |
|---|---|
| весь файл | все страницы и все объекты на них |
| только открытую страницу | та страница, которая открыта в Pixso в момент нажатия |
| только выделенное в Pixso | то, что выделено в Pixso в момент нажатия |

Выбор считывается **в момент нажатия**: дальше раннер уходит работать в Pixso и переспросить уже не
может. Выделять можно не только объекты верхнего уровня — вложенный фрейм тоже перенесётся и ляжет на
ту же страницу, с которой был взят, а не на ту, что случайно открыта в Figma.

Узкий выбор ещё и быстрее на входе: при переносе одной страницы или выделения остальной файл не
пересчитывается вовсе. На библиотеке в 48 страниц это разница между одним обращением к Pixso и
отказом по таймауту.

Извлечённое складывается в `out/<имя файла>`, поэтому два разных файла не перемешиваются.
...
| `tools/pix-open.mjs` | читает `.pix` с диска без Pixso — пока только читает, не переносит |
| `tools/kiwi.mjs` | разбор схемы и сообщений Kiwi, на которых устроен `.pix` |
